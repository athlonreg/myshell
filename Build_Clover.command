...
CLOVERV2_PATH="${DIR_MAIN}/edk2_micky/Clover/CloverPackage/CloverV2"
PKG_PATH="${DIR_MAIN}/edk2_micky/Clover/CloverPackage/package"
LOCALIZABLE_FILE="${PKG_PATH}/Resources/templates/Localizable.strings"
ebuildB="${DIR_MAIN}/edk2_micky/Clover/ebuildBorg.sh"
ebuild="${DIR_MAIN}/edk2_micky/Clover/ebuild.sh"
exportPaths
}
# --------------------------------------
setBuildTools() {
...
# --------------------------------------
CheckProprietary() {
local drivers_off="${DIR_MAIN}/edk2_micky/Clover/CloverPackage/CloverV2/drivers-Off"
...
if [[ -d "${DIR_MAIN}"/edk2_micky/Clover/.svn ]]; then
	svnUpgrade # upgrade the working copy to avoid errors
	LOCAL_REV=$(svn info "${DIR_MAIN}"/edk2_micky/Clover | grep '^Revision:' | tr -cd [:digit:])
else
	LOCAL_REV=""
fi
if [[ -d "${DIR_MAIN}"/edk2_micky/.svn ]]; then
	LOCAL_EDK2_REV=$(svn info "${DIR_MAIN}"/edk2_micky | grep '^Revision:' | tr -cd [:digit:])
else
	LOCAL_EDK2_REV=""
fi
}
...
do
	...
	TIMES=0
	IsLinkOnline $link
	if [[ -d "${DIR_MAIN}/edk2_micky/${pkg}" ]] ; then
		cd "${DIR_MAIN}/edk2_micky/${pkg}"
		if [[ -d "${DIR_MAIN}/edk2_micky/${pkg}/.svn" ]] ; then
			local localRev=$(svn info "${DIR_MAIN}/edk2_micky/${pkg}" | grep '^Revision:' | tr -cd [:digit:])
...
cd "${DIR_MAIN}"/edk2_micky
source edksetup.sh BaseTools
# Create edk tools if necessary
if [[ ! -x "${DIR_MAIN}/edk2_micky/BaseTools/Source/C/bin/GenFv" ]]; then
	make -C "${DIR_MAIN}"/edk2_micky/BaseTools CC="gcc -Wno-deprecated-declarations"
fi
...
for driver in "AptioFixPkg" "AppleSupportPkg"; do
	build -a X64 -b RELEASE -t $BUILDTOOL -n $ncpu -p "${DIR_MAIN}"/edk2_micky/"${driver}"/"${driver}".dsc
done
cd "${DIR_MAIN}"/edk2_micky/Clover
}
# --------------------------------------
edk2() {
...
if [[ "$ForceEDK2Update" -ne "1979" ]]; then
	if [[ ! -d "${DIR_MAIN}/edk2_micky/.svn" ]]; then ForceEDK2Update=1; fi
	for d in "${edk2array[@]}"
	do
		if [[ "$d" != Source && "$d" != Scripts ]]; then
			if [[ ! -d "${DIR_MAIN}/edk2_micky/${d}/.svn" ]]; then ForceEDK2Update=1; fi
		fi
	done
fi
...
if [[ "$ForceEDK2Update" -eq "0" ]]; then
	...
	echo
	if [[ ! -d "${DIR_MAIN}/edk2_micky" ]]; then
		printHeader 'Downloading edk2'
		mkdir -p "${DIR_MAIN}"/edk2_micky
	else
	...
	do
        if [[ "$d" != "Source" && "$d" != "Scripts" ]]; then
            ...
            cd "${DIR_MAIN}"/edk2_micky
            if [[ -d "${DIR_MAIN}/edk2_micky/${d}" ]] ; then
                if [[ -d "${DIR_MAIN}/edk2_micky/${d}/.svn" ]] ; then
                    cd "${DIR_MAIN}/edk2_micky/${d}"
	...
	if [[ "$ForceEDK2Update" -eq "1979" ]]; then
		...
		echo
		if [[ -d "${DIR_MAIN}/edk2_micky/Clover" ]]; then cd "${DIR_MAIN}/edk2_micky/Clover"; ./ebuild.sh cleanall -t $BUILDTOOL; fi
		if [[ -d "${DIR_MAIN}/edk2_micky/Clover/CloverPackage" ]]; then cd "${DIR_MAIN}/edk2_micky/Clover/CloverPackage"; make clean; fi
		for tpdrv in "AptioFixPkg" "AppleSupportPkg"; do
			if [[ -d "${DIR_MAIN}/edk2_micky/Build/${tpdrv}" ]]; then 
				rm -rf "${DIR_MAIN}/edk2_micky/Build/${tpdrv}"
			fi
		done
		FORCEREBUILD="-fr"
	fi
...
if [[ -z "$SUGGESTED_CLOVER_REV" ]]; then
	...
	if [[ ! -d "${DIR_MAIN}/edk2_micky/Clover" ]]; then
		...
		if [[ "${LOCAL_REV}" == "" ]]; then
			...
			rm -rf "${DIR_MAIN}"/edk2_micky/Clover/* > /dev/null 2>&1
			cmd="co -r $REMOTE_REV --non-interactive --trust-server-cert ${CLOVER_REP} ."
		else
...
cp -R "${DIR_MAIN}"/edk2_micky/Clover/Patches_for_EDK2/* "${DIR_MAIN}"/edk2_micky/

# in Lion cp cause error with subversion (comment this line and enable next)
# rsync -rv --exclude=.svn "${DIR_MAIN}"/edk2_micky/Clover/Patches_for_EDK2/ "${DIR_MAIN}"/edk2_micky
}
# --------------------------------------
needGETTEXT() {
local gettextPath=""
...
if [[ "$SYSNAME" == Darwin ]]; then
	...
	if needGETTEXT && [[ ! -x "${TOOLCHAIN_DIR}"/bin/gettext ]]; then
		...
		doSomething --run-script "${DIR_MAIN}"/edk2_micky/Clover/buildgettext.sh
	fi
	printHeader "mtoc check:"
	if [[ ! -x "${TOOLCHAIN_DIR}/bin/mtoc.NEW" ]]; then
		printWarning "mtoc not found, installing...\n"
		local mtocpath="$DIR_MAIN/edk2_micky/Clover/BuildTools/usr/local/bin"
...
if [[ -d "${DIR_MAIN}/edk2_micky/Clover/.svn" && "$INTERACTIVE" != "NO" ]] ; then
	...
	case $choice in
		...
		"build boot6/7 with -fr --std-ebda" )
			...
			echo && printf "build started at:\n${START_BUILD}\nfinished at\n$(date)\n\nDone!\n";;
		"build pkg" )
			cd "${DIR_MAIN}"/edk2_micky/Clover/CloverPackage
			START_BUILD=$(date)
...
if [[ "$INTERACTIVE" != "NO" ]]; then
	if [[ "$BUILD_FLAG" == "NO" ]]; then
		...
		if [[ -d "${DIR_MAIN}"/edk2_micky ]]; then getRev; printRevisions; fi;
		if [[ $cbuildflag == 0 ]] ; then cbuild 5; fi
	fi
fi
...
# Delete useless themes
rm -rf ~/src/edk2_micky/Clover/CloverPackage/CloverV2/themespkg/* 
cp -R ~/Documents/Tower/Build_Clover/themes/* ~/src/edk2_micky/Clover/CloverPackage/CloverV2/themespkg/ 
...
cp -R ~/themes/* ~/src/edk2_micky/Clover/CloverPackage/CloverV2/themespkg/ 
...
if [[ "${cus_conf}" != "Y" ]]; then
	if [[ "${BUILD_CLOVER_CFG_PATH:-}" ]]; then
		...
		userconf="$HOME/BuildCloverConfig_Micky.txt"
	fi
fi
