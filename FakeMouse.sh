...
system_profiler SPUSBDataType 1> /tmp/usb.txt 2>/dev/null
mouses=$(cat /tmp/usb.txt |grep -i "mouse")
num=$(echo "${mouses}"|wc -l)
make_kext(){
mkdir -p ~/Desktop/FakeAppleUSBMouse.kext/Contents
read pid vid <<< $(printf %d\\n $(cat /tmp/usb.txt|grep -A3 "${mouse}"|grep ID|grep -o "0x\w*"))
rm -rf /tmp/usb.txt
if [ x"$pid" != "x0" ];then
cat <<'EOF'|sed "s/PPPID/${pid}/g;s/VVVID/${vid}/g" > ~/Desktop/FakeAppleUSBMouse.kext/Contents/Info.plist
...
<dict>
	...
	<string>com.taolu.injector.FakeAppleUSBMouse</string>
	<key>CFBundleInfoDictionaryVersion</key>
	<string>6.0</string>
	<key>CFBundleName</key>
	<string>FakeAppleUSBMouse</string>
	...
	<dict>
		...
		<dict>
			...
			<key>ProductID</key>
			<integer>PPPID</integer>
			<key>VendorID</key>
			<integer>VVVID</integer>
		</dict>
	</dict>
	<key>NSHumanReadableCopyright</key>
	<string>FakeAppleUSBMouse 9.9 Copyright © 2017 taolu. All rights reserved.</string>
</dict>
</plist>
EOF
	echo "FakeAppleUSBMouse.kext 生成完毕, 已放置到您的桌面上了."
else
	echo "数据提取失败, 请尝试更换接口并尝试再次运行脚本."
fi
}
if [ "$num" -lt 1 ];then
	echo "未找到任何鼠标设备, 请检查连接并尝试再次运行脚本!"
elif [ "$num" -gt 1 ];then
	n=0
	while (( $n < $num ))
	do
		let n++
		printf "[$n] "; echo "${mouses}"|sed -n ${n}p|sed 's/^ *//g;s/://g'
	done
	read -p "请选择需要生成仿冒驱动的设备编号: " choice
	mouse=$(echo "${mouses}"|sed -n ${choice}p)
	make_kext
else
	mouse=$mouses
	make_kext
